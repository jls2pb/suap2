...
if(isset($_SESSION['cpf_cer'])){
    ...
    $query = "SELECT * FROM agendamento WHERE status = 0 AND data_atendimento = :hoje AND local_atendimento = 'CER MARIA DA CONCEICAO RODRIGUES DE ANDRADE' AND nome_paciente LIKE '%$dado%' ORDER BY nome_paciente ASC LIMIT $limite_resultado OFFSET $inicio";
...
$query_usuarios = "SELECT DISTINCT ON (nome_paciente) * FROM agendamento  WHERE local_atendimento = 'CER MARIA DA CONCEICAO RODRIGUES DE ANDRADE' AND nome_paciente LIKE '%$dado%' ORDER BY nome_paciente ASC LIMIT $limite_resultado OFFSET $inicio";
