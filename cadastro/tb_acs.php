...
<table class="table table-striped table-compact" style="font-size:12px;">
        ...
        <tbody>
            ...
            <td>
                <a class="btn text-white btn-primary" href="edita_acs.php?id=<?php echo $d['cod'];?>" role="button" style="font-size:10px;"><b>EDITAR</b></a>
            </td>
            <td>
                <a class="btn text-white btn-danger" onclick="confirmarExclusao(<?php $_GET['id'] = $d['cod']; echo $_GET['id']; ?>)" role="button" style="font-size:10px;"><b>EXCLUIR</b></a>
            </td>
            </tr>
            <?php
                    }
            ...
            if ($pagina > 1) {
                ...
                echo "<a href='tb_acs.php?page=$pagina_anterior&cpf=$cpf_logado' class='btn'><label style='font-size:30px;' title='Anterior'><span aria-hidden='true'>&laquo;</span></label></a> ";
            }
    
            if ($pagina < $qnt_pagina) {
                $proxima_pagina = $pagina + 1;
                echo "<a href='tb_acs.php?page=$proxima_pagina&cpf=$cpf_logado' class='btn'><label style='font-size:30px;' title='Próximo'><span aria-hidden='true'>&raquo;</span></label></a> ";
            }
    
            if ($pagina < $qnt_pagina) {
                ?><div class="float-right"><?php
                echo "<a class='btn' style='color: white; background-color: #66a7ff;' href='tb_acs.php?page=$qnt_pagina&cpf=$cpf_logado'>Última</a> ";
            ?></div> <?php
            }
