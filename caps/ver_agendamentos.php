...
<table class="table table-striped table-bordered table-sm table-responsive">
        ...
        <tbody>
          ...
          <div class = "row">
            ...
            <?php 
               if ($pagina > 1) {
                echo "<a class='btn' style='color: white; background-color: #66a7ff;' href='ver_agendamento.php?page=1&cpf=$cpf_logado'>Primeira</a> ";
            }
    
            if ($pagina > 1) {
                $pagina_anterior = $pagina - 1;
                echo "<a href='ver_agendamento.php?page=$pagina_anterior&cpf=$cpf_logado'><label style='font-size:30px;' title='Anterior'>&#8592</label></a> ";
            }
    
            if ($pagina < $qnt_pagina) {
                $proxima_pagina = $pagina + 1;
                echo "<a href='ver_agendamento.php?page=$proxima_pagina&cpf=$cpf_logado'><label style='font-size:30px;' title='Próximo'>&#8594</label></a> ";
            }
    
            if ($pagina < $qnt_pagina) {
                ?><div class="float-right"><?php
                echo "<a class='btn' style='color: white; background-color: #66a7ff;' href='ver_agendamento.php?page=$qnt_pagina&cpf=$cpf_logado'>Última</a> ";
            ?></div> <?php
            }
